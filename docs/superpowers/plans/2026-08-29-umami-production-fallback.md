# TodoPlásticos Umami production fallback plan

**Goal:** Ensure TodoPlásticos always emits its Agama Umami tracker after a Coolify build, even when the two public build variables are missing.
...
**Architecture:** Keep `next/script`, centralize the non-secret Agama URL and TodoPlásticos website ID in a validated public resolver, and reuse it in the tracker and server statistics clients. Environment overrides remain accepted only for the same Agama host and a valid UUID.
...
- [x] Restrict tracker collection to the canonical TodoPlásticos domains.
