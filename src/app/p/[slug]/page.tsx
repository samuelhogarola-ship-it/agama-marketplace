...
function ContactActions({ product, primary = false }: { product: Product; primary?: boolean }) {
  const listingContactHref = contactHref(product.contact_override?.method, product.contact_override?.value);
  return (
    <div className={primary ? "mt-5 space-y-3" : "mt-4 flex flex-wrap gap-2 text-sm"}>
      {listingContactHref ? (
        <a
          href={listingContactHref}
          target={product.contact_override?.method === "whatsapp" ? "_blank" : undefined}
          rel="noreferrer"
          className={primary ? "block rounded-full bg-brand px-5 py-3 text-center text-sm font-semibold text-white hover:bg-brand-dark" : "rounded-full bg-brand px-3 py-1.5 text-white hover:bg-brand-dark"}
        >
      ...
      {product.external_url ? (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className={primary ? "block rounded-full border border-brand px-5 py-3 text-center text-sm font-semibold text-brand-dark hover:bg-brand-light" : "rounded-full border border-brand px-3 py-1.5 text-brand-dark hover:bg-brand-light"}
        >
...
function SummaryPanel({ product, categoryName }: { product: Product; categoryName?: string }) {
  return (
    <aside className="rounded-[8px] border border-slate-200 bg-white p-5 shadow-sm">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-sky">{categoryName ?? "Anuncio B2B"}</p>
      <h1 className="mt-3 text-2xl font-semibold leading-tight tracking-[-0.03em] text-brand-dark">{product.title}</h1>
      <p className="mt-4 text-3xl font-semibold tracking-[-0.03em] text-brand-dark">
        {formatPrice(product.price_mxn)}
        {product.unit && product.price_mxn !== null ? <span className="text-base font-normal text-slate-500"> / {product.unit}</span> : null}
      </p>
      <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
        <div className="rounded-[6px] bg-slate-50 px-3 py-3">
          <p className="text-xs text-slate-400">Compra mínima</p>
          <p className="mt-1 font-semibold text-brand-dark">{product.min_purchase_qty ?? 1} {product.unit ?? "unidad"}</p>
        </div>
        <div className="rounded-[6px] bg-slate-50 px-3 py-3">
          <p className="text-xs text-slate-400">Ubicación</p>
          <p className="mt-1 font-semibold text-brand-dark">{product.location ?? "México"}</p>
        </div>
      </div>
      <ContactActions product={product} primary />
      <p className="mt-4 text-xs leading-5 text-slate-400">TodoPlástico no procesa pagos ni intermedia operaciones. El acuerdo se realiza directamente entre empresas.</p>
    </aside>
  );
}
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-8 lg:px-12">
      ...
      <nav className="text-sm text-slate-500">
        ...
        {" / "}
        {cat ? <Link href={`/c/${cat.slug}`} className="hover:text-brand">{cat.name}</Link> : "Anuncio"}
      </nav>
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start">
        <section className="min-w-0">
          <div className="overflow-hidden rounded-[8px] bg-slate-100">
            <div className="aspect-[16/10] bg-slate-100">
              {photos[0] ? (
                ...
                <img src={photoUrl(photos[0].storage_path)} alt={product.title} className="h-full w-full object-cover" />
              ) : (
                <div className="flex h-full w-full items-center justify-center text-5xl text-slate-300">TP</div>
              )}
            </div>
          </div>
          ...
          {photos.length > 1 ? (
            <div className="mt-3 grid grid-cols-5 gap-2">
              {photos.slice(1, 6).map((photo) => (
                <div key={photo.id} className="aspect-square overflow-hidden rounded-[6px] bg-slate-100">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          <div className="mt-6 lg:hidden">
            <SummaryPanel product={product} categoryName={cat?.name} />
          </div>

          <section className="mt-8 border-t border-slate-200 py-6">
            <h2 className="text-lg font-semibold text-brand-dark">Descripción</h2>
            <p className="mt-4 whitespace-pre-line text-base leading-8 text-slate-600">{product.description}</p>
          </section>

          <section className="border-t border-slate-200 py-6">
            <h2 className="text-lg font-semibold text-brand-dark">Características</h2>
            <dl className="mt-4 grid gap-4 sm:grid-cols-2">
              <div className="flex justify-between gap-4 border-b border-slate-100 pb-3">
                <dt className="text-sm text-slate-400">Categoría</dt>
                <dd className="text-right text-sm font-semibold text-brand-dark">{cat?.name ?? product.category}</dd>
              </div>
              <div className="flex justify-between gap-4 border-b border-slate-100 pb-3">
                <dt className="text-sm text-slate-400">Tipo</dt>
                <dd className="text-right text-sm font-semibold text-brand-dark">{product.type === "product" ? "Producto" : product.type === "service" ? "Servicio" : "Anuncio B2B"}</dd>
              </div>
              <div className="flex justify-between gap-4 border-b border-slate-100 pb-3">
                <dt className="text-sm text-slate-400">Cómo se vende</dt>
                <dd className="text-right text-sm font-semibold text-brand-dark">{product.unit ?? "unidad"}</dd>
              </div>
              <div className="flex justify-between gap-4 border-b border-slate-100 pb-3">
                <dt className="text-sm text-slate-400">Compra mínima</dt>
                <dd className="text-right text-sm font-semibold text-brand-dark">{product.min_purchase_qty ?? 1}</dd>
              </div>
            </dl>
          </section>
          ...
          <section className="border-t border-slate-200 py-4 text-sm text-slate-400">
            <span>{product.updated_at ? `Actualizado ${formatDate(product.updated_at)}` : `Publicado ${formatDate(product.created_at)}`}</span>
          </section>
        </section>
