...
export const metadata: Metadata = {
  title: "Artículos sobre la industria plástica",
  description: "Guías prácticas, procesos y referencias para profesionales de la industria plástica en México.",
};
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-14 sm:px-8 lg:px-12 lg:py-20">
      ...
      <Link href={`/articulos/${featured.slug}`} className="group mt-12 grid overflow-hidden rounded-[4px] bg-brand-dark text-white md:grid-cols-2">
        <div className="relative aspect-[4/3] overflow-hidden md:aspect-auto"><Image src={featured.cover} alt="" fill sizes="(min-width: 768px) 50vw, 100vw" className="object-cover transition-transform duration-700 group-hover:scale-[1.03]" /></div>
      ...
      <div className="mt-14 grid gap-x-6 gap-y-10 md:grid-cols-2 lg:grid-cols-4">
        {rest.map((article) => (
          <Link key={article.slug} href={`/articulos/${article.slug}`} className="group border-t border-slate-200 pt-4">
            <div className="relative aspect-[4/3] overflow-hidden rounded-[4px] bg-slate-100"><Image src={article.cover} alt="" fill sizes="(min-width: 1024px) 25vw, (min-width: 768px) 50vw, 100vw" className="object-cover transition-transform duration-700 group-hover:scale-[1.03]" /></div>
