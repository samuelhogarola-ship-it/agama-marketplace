...
Para activar la dirección estructurada, aplicar `supabase/migrations/0016_company_address.sql` al proyecto Supabase de TodoPlásticos (`tiynnllrcdhsvrzsdsct`). Es aditiva y compatible con la versión anterior. El formulario detecta la columna en la respuesta privada del propietario. Si aún no existe, conserva ubicación libre y no envía `address`: se puede fusionar y desplegar sin romper el guardado. Verificar la columna y sus restricciones después de aplicar la migración. La dirección completa no recibe permisos públicos de lectura; el propietario la lee por `mkt_my_company()`. `location` mantiene municipio y estado para el directorio existente.
