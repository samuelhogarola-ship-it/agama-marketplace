...
const FROM = "TodoPlásticos <contacto@xn--todoplsticos-hbb.es>";
...
export async function sendTicketCreatedToAdmin(opts: {
  ...
  await resend.emails.send({
    ...
    html: `
      <p>Se ha abierto un nuevo ticket de soporte en TodoPlásticos.</p>
      ...
      <p style="margin-top:16px"><a href="${process.env.NEXT_PUBLIC_SITE_URL ?? "https://todoplásticos.com"}/admin#tickets" style="background:#1e3a5f;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-size:14px">Ver ticket en el panel admin</a></p>
    `,
  });
}
...
export async function sendTicketReplyToUser(opts: {
  ...
  await resend.emails.send({
    ...
    html: `
      <p>El equipo de TodoPlásticos ha respondido a tu ticket <strong>${esc(opts.ticketCode)}</strong>.</p>
      ...
      <p><a href="${process.env.NEXT_PUBLIC_SITE_URL ?? "https://todoplásticos.com"}/panel#soporte" style="background:#1e3a5f;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-size:14px">Ver hilo completo</a></p>
      <p style="margin-top:24px;font-size:12px;color:#94a3b8">TodoPlásticos · Directorio B2B de la industria plástica en México</p>
    `,
  });
}
...
export async function sendTicketReplyToAdmin(opts: {
  ...
  await resend.emails.send({
    ...
    html: `
      ...
      <blockquote style="margin:16px 0;padding:12px 16px;border-left:3px solid #e2e8f0;color:#475569;font-size:14px">${escMultiline(opts.message)}</blockquote>
      <p><a href="${process.env.NEXT_PUBLIC_SITE_URL ?? "https://todoplásticos.com"}/admin#tickets" style="background:#1e3a5f;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-size:14px">Responder en el panel admin</a></p>
    `,
  });
}
